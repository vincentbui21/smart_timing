...
db = mysql.connector.connect(
    host="localhost", user="admin", password="123", database="std"
)
...
def get_runner_name(devide: bool = False):
    ...
    if devide:
        ...
        return (
            tuple(temp_tuple_name),
            tuple(temp_tuple_id),
            tuple(temp_tuple_mix_name_id),
        )
    else:
        return result
...
if __name__ == "__main__":
    ...
    mycursor.execute(
        f'SELECT timestamp FROM timestamphistory LEFT JOIN runners ON timestamphistory.runner_id = runners.id WHERE name ="{option}"'
    )
    result = mycursor.fetchall()
