    ...
    with st.expander("Choose sequence"):
        ...
        with col1:
            if st.button('Choose the sequence randomly!'):
                ...
                try:
                    ...
                    merged_list = [[list(st.session_state.Connection_List.values())[i], filtered_list[i]] for i in range(len(filtered_list))]
        ...
        with col2:
            for i in range(0, num_con):
                ...
                col1.button(label='Left', key=f'Left_{i}', on_click=update_sequence, args=(f'Left_{i}',))
                col2.button(label='Right', key=f'Right_{i}', on_click=update_sequence, args=(f'Right_{i}',))
...
def start():
    ...
    logic_thread = threading.Thread(target=logic.main_logic, args=(stop_event, conn_error_event, st.session_state.Connection_List, pause_queue, st.session_state.sequence_method,), daemon=True)
    logic_thread.start()
